...
def d2TSI_dV2dP2(Surrogate, Pg, Qg, Pl, Ql, muTSI, st_args):
    ...
    if Surrogate['model_type'] == "CNF":
        ...
        raise ValueError(
            f"Hessian evaluation failed: the selected surrogate model "
            f"({Surrogate['model_type']}) does not have an associated Hessian function. "
            f"Please implement the Hessian or choose a compatible surrogate."
        )
...
def d2TSI_dV2dP2_DKL(GPmodel, Pg, Qg, Pl, Ql, muTSI, st_args):
    ...
    if active_gen_only:
        ...
        X_part_np = np.hstack([Pg_active_rew, Pg_active_syn])
    else:
        Pg_syn = Pg[syn_idx].reshape(1, -1)
        Pg_rew = Pg[rew_idx].reshape(1, -1)

        X_part_np = np.hstack([Pg_rew, Pg_syn])
    ...
    def constraint_f_part(Xp):
        X_full = torch.cat([Xp, X_const], dim=1) * 100
    ...
    Hessian_part = Hessian_part.squeeze(0).squeeze(1)
    ...
    # H_new = np.zeros_like(Hessian_part_np)
    # H_new[np.ix_(gen_rewsyn_genidx, gen_rewsyn_genidx)] = Hessian_part_np
